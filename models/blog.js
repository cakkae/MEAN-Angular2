...
// Validate Function to check if valid amount
let amountValidatorChechker = (amount) => {
  // Check if amount exists
  if (!amount) {
    return false; // Return error
  } else {
    // Regular expression to test for a valid amount
    const regExp = new RegExp(/^[0-9 ]+$/);
    return regExp.test(amount); // Return regular expression test results (true or false)
  }
};
...
const blogSchema = new Schema({
  ...
  amount: { type: String, required: true, validate: alphaNumericTitleChecker },
